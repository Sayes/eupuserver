...
#ifndef NETWORK_SYSQUEUE_H_
#define NETWORK_SYSQUEUE_H_
...
class SysQueue {
 public:
  ...
  SysQueue() { m_maxSize = 15000; }

  void setQueueSize(int nsize) { m_maxSize = nsize; }
  ...
  int sizeWithoutLock() { return m_datalst.size(); }
  ...
  bool isEmptyWithoutLock() { return m_datalst.empty(); }
  ...
  bool inQueue(T *arg, bool bhead) {
    ...
    do {
      ...
      if (m_datalst.size() >= (uint32_t)m_maxSize) {
        break;
      }
  ...
  bool inQueueWithoutLock(T *arg, bool bhead) {
    ...
    if (m_datalst.size() >= (uint32_t)m_maxSize) return false;
  ...
  bool outQueueWithoutLock(T *&arg, bool bhead) {
    if (m_datalst.size() <= 0) return false;
    if (bhead) {
      arg = m_datalst.front();
      m_datalst.pop_front();
    } else {

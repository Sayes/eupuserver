...
bool Utility::Compress(uint8_t *pSrc, int32_t nLen, uint8_t *pDst,
                       int32_t &nDstLen) {
    // int32_t nCompleteLen = compress2(pDst,(uLongf*)&nDstLen,pSrc,nLen,5);
    return true;
}

bool Utility::Uncompress(uint8_t *pSrc, int32_t nLen, uint8_t *pDst,
                         int32_t &nDstLen) {
    return true;
}
...
void Utility::GetDate4Log(char *szDate) {
    ...
    sprintf(szDate, "%d%.2d%.2d", (1900 + ptm.tm_year), (1 + ptm.tm_mon),
            ptm.tm_mday);
#elif OS_WINDOWS
    if (szDate == NULL) {
        return;
    ...
    if (p)
        sprintf(szDate, "%d%.2d%.2d", (1900 + p->tm_year), (1 + p->tm_mon),
                p->tm_mday);
...
void Utility::GetDateTime4Log(char *szDateTime) {
    ...
    sprintf(szDateTime, "%d%.2d%.2d%.2d%.2d%.2d", (1900 + p.tm_year),
...
bool Utility::CheckDate(int32_t year, int32_t month, int32_t day) {
    ...
    if (tm_new.tm_year != (year - 1900) || tm_new.tm_mon != (month - 1) ||
        tm_new.tm_mday != day) {
        return false;
    } else {
        return true;
    ...
    if (p->tm_year != (year - 1900) || p->tm_mon != (month - 1) ||
        p->tm_mday != day) {
        return false;
    } else {
        return true;
...
int32_t Utility::Base64Decode(unsigned char *szSrc, unsigned char *szDst,
                              int32_t SrcSize) {
    int32_t i = 0, j = 0;
    ...
    for (i = 0; i < SrcSize; i += 4) {
        szDst[j++] = base64_decode_map[szSrc[i]] << 2 |
                     base64_decode_map[szSrc[i + 1]] >> 4;

        szDst[j++] = base64_decode_map[szSrc[i + 1]] << 4 |
                     base64_decode_map[szSrc[i + 2]] >> 2;

        szDst[j++] = base64_decode_map[szSrc[i + 2]] << 6 |
                     base64_decode_map[szSrc[i + 3]];
...
int32_t Utility::getIntDotNet(unsigned char *bb, int32_t index) {
    return (int32_t)(
        (((bb[index + 3] & 0xff) << 24) | ((bb[index + 2] & 0xff) << 16) |
         ((bb[index + 1] & 0xff) << 8) | ((bb[index + 0] & 0xff) << 0)));
}

int64_t Utility::getLongDotNet(unsigned char *bb, int32_t index) {
    return ((((int64_t)bb[index + 7] & 0xff) << 56) |
            (((int64_t)bb[index + 6] & 0xff) << 48) |
            (((int64_t)bb[index + 5] & 0xff) << 40) |
            (((int64_t)bb[index + 4] & 0xff) << 32) |
            (((int64_t)bb[index + 3] & 0xff) << 24) |
            (((int64_t)bb[index + 2] & 0xff) << 16) |
            (((int64_t)bb[index + 1] & 0xff) << 8) |
            (((int64_t)bb[index + 0] & 0xff) << 0));
}
...
std::string &Utility::ReplaceAll(std::string &str, const char *old_value,

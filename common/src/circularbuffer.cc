 ...
 * Based on the Bip Buffer concept, from
...
/** Destructor
 */
CircularBuffer::~CircularBuffer() { free(m_buffer); }
...
bool CircularBuffer::Read(void *destination, size_t bytes) {
  ...
  // If we have both region A and region B, always "finish" off region A
  // first,
  // as
  ...
  // is buffer A empty? move buffer B to buffer A, to increase future
  // performance
  if (m_regionASize == 0) {
    if (m_regionBSize > 0) {
      // push it all to the start of the buffer.
      if (m_regionBPointer != m_buffer) memmove(m_buffer, m_regionBPointer, m_regionBSize);
...
bool CircularBuffer::Write(const void *data, size_t bytes) {
  ...
  // Otherwise, write to buffer A, or initialize buffer B depending on which
  // has
  // more space.
 ...
 */
size_t CircularBuffer::GetSize() { return m_regionASize + m_regionBSize; }

/** Returns the number of contiguous bytes (that can be pushed out in one
 * operation)
 */
...
/** Returns a pointer at the "beginning" of the buffer, where data can be pulled
 * from
 */

...
void CEupuLogger4System::WriteMonitorLog(uint32_t type, uint32_t mainid,
                                         uint32_t assiantid, uint32_t action,
                                         const char *username,
                                         const char *domain) {}
...
// get the log string
  ...
  snprintf(buf, sizeof(buf), "%04d/%02d/%02d %02d:%02d:%02d:%06u [0x%08x, "
                             "0x%08x] <%-5s> (%s, %06d) %s",
...
void CEupuLogger4System::WriteMonitorLog(uint32_t type, uint32_t mainid,
  ...
  snprintf(buf, sizeof(buf),
           "%04d-%02d-%02d %02d:%02d:%02d---%u---%s---%s---%s---%u",
           tme.tm_year + 1900, tme.tm_mon + 1, tme.tm_mday, tme.tm_hour,
           tme.tm_min, tme.tm_sec, type, GETNULLSTR(strkey),
           GETNULLPTR(username), GETNULLPTR(domain), action);
  ...
  snprintf(buf, sizeof(buf),
           "%04d-%02d-%02d %02d:%02d:%02d---%u---%s---%s---%s---%u",
           p->tm_year + 1900, p->tm_mon + 1, p->tm_mday, p->tm_hour, p->tm_min,
           p->tm_sec, type, GETNULLSTR(strkey), GETNULLPTR(username),
           GETNULLPTR(domain), action);
...
void CEupuLogger4System::WriteHex(const char *filename, int32_t line,
  ...
  int32_t pid = (int32_t)getpid();
  snprintf(msgbuf, sizeof(msgbuf), "%04d/%02d/%02d %02d:%02d:%02d:%06u "
  ...
  int32_t threadid = 0;
  snprintf(
      msgbuf, sizeof(msgbuf), "%04d/%02d/%02d %02d:%02d:%02d:%06u [0x%08x, "
                              "0x%08x] <%-5s> (%s, %06d) %s: \n%s",
      p->tm_year + 1900, p->tm_mon + 1, p->tm_mday, p->tm_hour, p->tm_min,
      p->tm_sec, (uint32_t)tmv.tv_usec, pid, threadid, GetLogLevelStr(level),
      filename, line, GETNULLPTR(title), GETNULLSTR(hexdata));
#endif

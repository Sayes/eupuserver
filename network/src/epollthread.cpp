...
CEpollThread::CEpollThread()
    : CEupuThread(), m_epollfd(-1), m_listenfd(-1), m_listenkey(NULL),
      m_keepalivetimeout(120), m_keepaliveinterval(60), m_serverip(""),
      m_serverport(0), m_readbufsize(0), m_sendbufsize(0), m_recvbuffer(NULL),
      m_recvbuflen(0), m_events(NULL), m_maxepollsize(5000), m_index(0) {
...
bool CEpollThread::startup() {
  ...
  if (m_epollfd < 0) {
    LOG(_ERROR_, "CEpollThread::startup() error, epoll_create() failed, "
  ...
  if (!doListen()) {
    LOG(_ERROR_,
        "CEpollThread::startup() error, doListen() failed, m_epollfd=%d",
        m_epollfd);
    return false;
  }

  if (!start()) {
    LOG(_ERROR_, "CEpollThread::startup() error, start() failed, m_epollfd=%d",
        m_epollfd);
    return false;
  }
...
time_t CEpollThread::getIndex() {
  m_index++;
  if ((uint32_t)m_index == 0xFFFFFFFF)
    m_index = 1;
  return m_index;
}
...
void CEpollThread::doEpollEvent() {
  ...
  while (m_bOperate) {
    ...
    for (int i = 0; i < nevent; ++i) {
      ...
      if (!pkey) {
        LOG(_ERROR_, "CEpollThread::doEpollEvent() error, "
      ...
      if (m_events[i].events & EPOLLIN) {
        ...
        LOG(_ERROR_, "CEpollThread::doEpollEvent() error, epoll recv unknown "
    ...
    for (std::map<int, std::list<NET_DATA *> *>::iterator itersendmap =
      ...
      if (itersockmap == m_socketmap.end() || itersockmap->second == NULL ||
        ...
        LOG(_ERROR_, "CEpollThread::doEpollEvent() error, m_socketmap.find(fd) "
      ...
      if (epoll_ctl(m_epollfd, EPOLL_CTL_MOD, itersendmap->first, &ev) < 0) {
        LOG(_ERROR_, "CEpollThread::doEpollEvent() error, epoll_ctl() failed, "
                     "fd=%d, error=%s",
            itersendmap->first, strerror(errno));
        m_delsendfdlist.push_back(itersendmap->first);
        closeClient(itersockmap->second->key->fdkey,
    ...
    if (itersockmap != m_socketmap.end()) {
      if (itersockmap->second != NULL) {
        if (itersockmap->second->key != NULL) {
          bclosed = true;
          LOG(_ERROR_, "CEpollThread::doKeepaliveTimeout() error, close "
                       "socket_set for timeout, fd=%d, time=%u, peerip=%s, "
                       "port=%d",
              itersockmap->first, itersockmap->second->key->connect_time,
              GETNULLSTR(itersockmap->second->peer_ip),
              itersockmap->second->peer_port);
...
bool CEpollThread::doListen() {
  ...
  do {
    ...
    int tmp = 1;
    if (setsockopt(m_listenfd, SOL_SOCKET, SO_REUSEADDR, &tmp, sizeof(tmp)) <
        0) {
    ...
    if (bind(m_listenfd, reinterpret_cast<sockaddr *>(&serv_addr),
             sizeof(serv_addr)) < 0) {
    ...
    LOG(_INFO_, "CEpollThread::doListen() successed, listen and add to epoll "
...
bool CEpollThread::doAccept(int fd) {
  ...
  while (true) {
    ...
    if (connfd < 0) {
      if (errno != EAGAIN) {
        LOG(_ERROR_,
            "CEpollThread::doAccept(fd) accept(fd) error, fd=%d, errno=%s", fd,
            strerror(errno));
      }
      return true;
    }
    ...
    LOG(_INFO_, "CEpollThread::doAccept(), peerip=%s, port=%d",
        GETNULLSTR(peerip), port);

    if (!setNonBlock(connfd)) {
      LOG(_ERROR_, "CEpollThread::doAccept(fd) setNonBlock(fd) error, fd=%d, "
                   "peerip=%s, port=%d",
    ...
    if (setsockopt(connfd, SOL_SOCKET, SO_SNDBUF, &m_sendbufsize,
                   sizeof(m_sendbufsize)) < 0) {
      LOG(_ERROR_, "CEpollThread::doAccept(fd) setsockopt(connfd...) error, "
    ...
    if (setsockopt(connfd, SOL_SOCKET, SO_RCVBUF, &m_readbufsize,
                   sizeof(m_readbufsize)) < 0) {
      LOG(_ERROR_, "CEpollThread::doAccept(fd) setsockopt(connfd...) error, "
                   "connfd=%d, peerip=%s, port=%d, error=%s",
          connfd, peerip.c_str(), port, strerror(errno));
      close(connfd);
    ...
    if (!psockset) {
      LOG(_ERROR_, "CEpollThread::doAccept(fd) initSocketset(connfd...) error, "
                   "connfd=%d, peerip=%s, port=%d, error=%s",
          connfd, peerip.c_str(), port, strerror(errno));
      close(connfd);
      continue;
    ...
    if (!createConnectServerMsg(psockset)) {
      LOG(_ERROR_, "CEpollThread::doAccept(fd) "
          ...
          GETNULLSTR(psockset->peer_ip), psockset->peer_port, psockset->type);
      close(connfd);
      delete psockset;
      continue;
    }
    ...
    if (!addClientToEpoll(psockset)) {
      LOG(_ERROR_, "CEpollThread::doAccept(fd) addClientToEpoll(psockset) "
...
bool CEpollThread::addClientToEpoll(SOCKET_SET *psockset) {
  ...
  if (epoll_ctl(m_epollfd, EPOLL_CTL_ADD, psockset->key->fdkey, &ev) < 0) {
    LOG(_ERROR_,
        "CEpollThread::addClientToEpoll() epoll_ctl() error, fd=%d, error=%s",
        psockset->key->fdkey, strerror(errno));
    return false;
  }

  if (!addSocketToMap(psockset)) {
    LOG(_ERROR_, "CEpollThread::addClientToEpoll() addSocketToMap() error, "
                 "fd=%d, peerip=%s, port=%d",
        psockset->key->fdkey, GETNULLSTR(psockset->peer_ip),
        psockset->peer_port);
    return false;
  }
...
void CEpollThread::doRecvMessage(SOCKET_KEY *pkey) {
  ...
  std::unordered_map<int, SOCKET_SET *>::iterator iter =
      m_socketmap.find(pkey->fdkey);

  if (iter == m_socketmap.end() || iter->second == NULL ||
      iter->second->key == NULL) {
    LOG(_ERROR_, "CEpollThread::doRecvMessage() fd not found in m_socketmap, "
                 "fd=%d, time=%u",
        pkey->fdkey, pkey->connect_time);
    closeClient(pkey->fdkey, pkey->connect_time);
    return;
  ...
  if (iter->second->key != pkey) {
    LOG(_ERROR_, "CEpollThread::doRecvMessage() the found socket doesn't "
  ...
  while (1) {
    ...
    if (nret < 0) {
      LOG(_ERROR_, "CEpollThread::doRecvMessage() recv_msg() error, fd=%d, "
    ...
    if (buflen > 0) {
      ...
      if (!bparse) {
        LOG(_ERROR_, "CEpollThread::doRecvMessage() parsePacketToRecvQueue() "
    ...
    if (nret == 0) {
      LOG(_INFO_, "the peer have closed the connection, fd:%d, time:%u, "
                  "peerip:%s, port:%d",
          pkey->fdkey, pkey->connect_time, GETNULLSTR(iter->second->peer_ip),
          iter->second->peer_port);
    }
...
int CEpollThread::doSendMessage(SOCKET_KEY *pkey) {
  ...
  std::unordered_map<int, SOCKET_SET *>::iterator itermap =
      m_socketmap.find(pkey->fdkey);

  if (itermap == m_socketmap.end() || itermap->second == NULL ||
      itermap->second->key == NULL) {
    LOG(_ERROR_, "CEpollThread::doSendMessage() error, not found socket, fd=%d",
        pkey->fdkey);
  ...
  if (itersend == psendmap->end()) {
    LOG(_WARN_,
        "CEpollThread::doSendMessage() not found the socket %d at send msg map",
        pkey->fdkey);
    return 0;
  }
  ...
  if (itersend->second == NULL) {
    LOG(_DEBUG_,
        "CEpollThread::doSendMessage(), the send message list is empty");
  ...
  while (itersend->second->size() > 0) {
    ...
    if (!pdata) {
      LOG(_WARN_,
          "CEpollThread::doSendMessage(), send message data is empty, fd=%d",
          pkey->fdkey);
    ...
    if (!((itermap->second->key->fdkey == pdata->fddat) &&
      ...
      LOG(_ERROR_, "CEpollThread::doSendMessage(), send message data connect "
    ...
    if (nsend < 0) {
      LOG(_ERROR_, "CEpollThread::doSendMessage() send_msg() error, fd=%d, "
                   "time=%u, peerip=%s, port=%d, err=%s",
          pdata->fddat, pdata->connect_time,
          GETNULLSTR(itermap->second->peer_ip), itermap->second->peer_port,
          strerror(errno));
      ...
      if (num > 0) {
        LOG(_INFO_, "CEpollThread::doSendMessage() send part message success, "
        ...
        LOG(_INFO_, "CEpollThread::doSendMessage() send message success, "
                    "fd=%d, time=%u, peerip=%s, port=%d",
            pdata->fddat, pdata->connect_time,
            GETNULLSTR(itermap->second->peer_ip), itermap->second->peer_port);
      ...
      LOG(_INFO_, "CEpollThread::doSendMessage() send message success, fd=%d, "
                  "time=%u, peerip=%s, port=%d",
          pdata->fddat, pdata->connect_time,
          GETNULLSTR(itermap->second->peer_ip), itermap->second->peer_port);
      LOGHEX(_DEBUG_, "send message:", pdata->pdata, buflen);
...
bool CEpollThread::parsePacketToRecvQueue(SOCKET_SET *psockset, char *buf,
                                          int buflen) {
  ...
  while (curpos < buflen) {
    ...
    if (nlen > MAX_SEND_SIZE || nlen < NET_HEAD_SIZE) {
      LOG(_ERROR_, "CEpollThread::parsePacketToRecvQueue() error, invalid "
                   "message length, fd=%d, peerip=%s, port=%d, len=%d",
          psockset->key->fdkey, GETNULLSTR(psockset->peer_ip),
          psockset->peer_port, nlen);
      return false;
    }
    ...
    if (!pdata) {
      LOG(_ERROR_, "CEpollThread::parsePacketToRecvQueue() error, create "
    ...
    if (!pdata->init(psockset->key->fdkey, psockset->key->connect_time,
                     ...
                     nlen)) {
      LOG(_ERROR_, "CEpollThread::parsePacketToRecvQueue() error, "
...
void CEpollThread::closeClient(int fd, time_t conntime) {
  ...
  if (epoll_ctl(m_epollfd, EPOLL_CTL_DEL, fd, NULL) < 0) {
    LOG(_ERROR_, "CEpollThread::closeClient() error, delete socket from epoll "
                 "failed, fd=%d, error=%s",
        fd, strerror(errno));
  }
  ...
  if (iter == m_socketmap.end()) {
    LOG(_ERROR_,
        "CEpollThread::closeClient() error, can't find socket in map, fd=%d",
        fd);
  } else {
    if (iter->second != NULL) {
...
void CEpollThread::createClientCloseMsg(SOCKET_SET *psockset) {
  if ((psockset == NULL) || (psockset->key == NULL)) {
    LOG(_ERROR_,
        "CEpollThread::createClientCloseMsg() error, param psockset == NULL");
    return;
  }
  ...
  if (!msg.Out(reinterpret_cast<BYTE *>(buf), buflen)) {
    LOG(_ERROR_, "CEpollThread::createClientCloseMsg() error, msg.Out(buf, "
  ...
  NET_DATA *pdata = new NET_DATA;
  if (!pdata) {
    LOG(_ERROR_, "CEpollThread::createClientCloseMsg() error, create NET_DATA "
  ...
  if (!pdata->init(psockset->key->fdkey, psockset->key->connect_time,
                   psockset->peer_ip, psockset->peer_port, psockset->type,
                   buflen)) {
    LOG(_ERROR_, "CEpollThread::createClientCloseMsg() error, "
...
bool CEpollThread::addSocketToMap(SOCKET_SET *psockset) {
  if (psockset == NULL || psockset->key == NULL || psockset->key->fdkey < 0) {
    LOG(_ERROR_,
        "CEpollThread::addSocketToMap() error, param psockset == NULL");
    return false;
  }

  if ((uint32_t)m_maxepollsize <= m_socketmap.size()) {
    LOG(_ERROR_, "CEpollThread::addSocketToMap() error, the epoll poll is "
  ...
  if (iter != m_socketmap.end()) {
    if (iter->second && iter->second->key) {
      LOG(_WARN_, "CEpollThread::addSocketToMap() error, found timeout "
...
void CEpollThread::deleteSendMsgFromSendMap(int fd) {
  if (fd < 0) {
    LOG(_ERROR_,
        "CEpollThread::deleteSendMsgFromSendMap(int fd) error, invalid fd");
    return;
  }
  ...
  while (pevent->outQueueWithoutLock(pdata, true)) {
    ...
    case CLOSE_CLIENT: {
      if (pdata->data != NULL) {
        ...
        LOG(_INFO_,
            "CEpollThread::doSystemEvent(), closeClient(), fd=%d, conn_time=%u",
            pkey->fdkey, pkey->connect_time);
        closeClient(pkey->fdkey, pkey->connect_time);
        delete pkey;
      }
    ...
    case ADD_CLIENT: {
      if (pdata->data == NULL) {
        LOG(_ERROR_,
            "CEpollThread::doSystemEvent(), ADD_CLIENT event data is null");
        break;
      }
      SOCKET_SET *psockset = reinterpret_cast<SOCKET_SET *>(pdata->data);
      if (psockset->key == NULL) {
        LOG(_ERROR_,
            "CEpollThread::doSystemEvent(), ADD_CLIENT socket->key is null");
        delete psockset;
        break;
      }
      ...
      if (!addClientToEpoll(psockset)) {
        LOG(_ERROR_, "CEpollThread::doSystemEvent() error, addClientToEpoll() "
    ...
    default: {
      ...
      if (pdata->data != NULL)
        delete reinterpret_cast<char *>(pdata->data);
      break;
    }
    } // end switch
    delete pdata;
...
bool CEpollThread::createConnectServerMsg(SOCKET_SET *psockset) {
  if ((psockset == NULL) || (psockset->key == NULL)) {
    LOG(_ERROR_, "CEpollThread::createConnectServerMsg() error, param "
                 "SOCKET_SET* is NULL");
    return false;
  }
  ...
  if (!msg.Out(reinterpret_cast<BYTE *>(buf), buflen)) {
    LOG(_ERROR_, "CEpollThread::createConnectServerMsg() error, msg.Out() "
  ...
  NET_DATA *pdata = new NET_DATA;
  if (pdata == NULL) {
    LOG(_ERROR_, "CEpollThread::createConnectServerMsg() error, new NET_DATA "
                 "failed, fd=%d, conn_time=%u, peer_ip=%s, port=%d",
        psockset->key->fdkey, psockset->key->connect_time,
        GETNULLSTR(psockset->peer_ip), psockset->peer_port);
    ::exit(-1);

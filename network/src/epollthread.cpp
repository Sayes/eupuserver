...
void CEpollThread::doEpollEvent()
{
    ...
    while (m_bOperate)
    {
        ...
        for (int i = 0; i < nevent; ++i)
        {
            ...
            if (m_events[i].events & EPOLLIN)
            {
                if (pkey->fdkey == m_listenfd)
                {
                    ...
                    {
                        LOG(_ERROR_, "CEpollThread::doEpollEvent() error, doAccept(pkey->fdkey) failed, fd=%d", pkey->fdkey);
                    }
                    continue;
                }
            ...
            else if (m_events[i].events & EPOLLOUT)
            {
                ...
                else
                {
                    ...
                    {
                        LOG(_ERROR_, "CEpollThread::doEpollEvent() error, epoll_ctl(m_epollfd, EPOLL_CTL_MOD, pkey->fdkey, &wev) failed, fd=%d", pkey->fdkey);
                        closeClient(pkey->fdkey, pkey->connect_time);
                    }
                }
            }
            ...
            {
                LOG(_ERROR_, "CEpollThread::doEpollEvent() error, epoll recv unknown error, fd=%d, conn_time=%u", pkey->fdkey, pkey->connect_time);
                closeClient(pkey->fdkey, pkey->connect_time);
            }
        }//end for
        ...
        //////////////////end set write epoll event by sendset///////////////////////////////
...
void CEpollThread::doKeepaliveTimeout()
{
    ...
    for (list<int>::iterator itertimeout = timelist.begin(); itertimeout != timelist.end(); ++itertimeout)
    {
        ...
        if (itersockmap != m_socketmap.end())
        {
            if (itersockmap->second != NULL)
            {
                if (itersockmap->second->key != NULL)
                {
                    ...
                    LOG(_ERROR_, "CEpollThread::doKeepaliveTimeout() error, close socket_set for timeout, fd=%d, time=%u, peerip=%s, port=%d",
                        itersockmap->first, itersockmap->second->key->connect_time, GETNULLSTR(itersockmap->second->peer_ip), itersockmap->second->peer_port);
            ...
            if (!bclosed)
            {
                LOG(_ERROR_, "CEpollThread::doKeepaliveTimeout() error, close socket_set->key->fdkey close for timeout");
    ...
    while (true)
    {
        ...
        if (!createConnectServerMsg(psockset))
        {
            LOG(_ERROR_, "CEpollThread::doAccept(fd) createConnectServerMsg(psockset) error, connfd=%d, conntime=%u, peerip=%s, port=%d, type=%d",
                psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip), psockset->peer_port, psockset->type);
            close(connfd);
            delete psockset;
            continue;
        }
...
bool CEpollThread::addClientToEpoll(SOCKET_SET* psockset)
{
    ...
    {
        LOG(_ERROR_, "CEpollThread::addClientToEpoll() epoll_ctl() error, fd=%d, error=%s", psockset->key->fdkey, strerror(errno));
        return false;
    }

    if (!addSocketToMap(psockset))
    {
        LOG(_ERROR_, "CEpollThread::addClientToEpoll() addSocketToMap() error, fd=%d, peerip=%s, port=%d", psockset->key->fdkey, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
        return false;
    }
...
void CEpollThread::doRecvMessage(SOCKET_KEY* pkey)
{
    ...
    {
        LOG(_ERROR_, "CEpollThread::doRecvMessage() fd not found in m_socketmap, fd=%d, time=%u", pkey->fdkey, pkey->connect_time);
        closeClient(pkey->fdkey, pkey->connect_time);
        return;
    }
    ...
    {
        LOG(_ERROR_, "CEpollThread::doRecvMessage() the found socket doesn't match, fd=%d, cur=%p, old=%p", pkey->fdkey, pkey, iter->second->key);
        closeClient(pkey->fdkey, pkey->connect_time);
        return;
    }
    ...
    while (1)
    {
        ...
        if (nret < 0)
        {
            LOG(_ERROR_, "CEpollThread::doRecvMessage() recv_msg() error, fd=%d, time=%u, peerip=%s, port=%d", pkey->fdkey, pkey->connect_time, GETNULLSTR(iter->second->peer_ip), iter->second->peer_port);
            closeClient(pkey->fdkey, pkey->connect_time);
            return;
        }
        ...
        if (buflen > 0)
        {
            ...
            if (!bparse)
            {
                LOG(_ERROR_, "CEpollThread::doRecvMessage() parsePacketToRecvQueue() error, fd=%d, time=%u, peerip=%s, port=%d",
                    pkey->fdkey, pkey->connect_time, GETNULLSTR(iter->second->peer_ip), iter->second->peer_port);
            }
        }
...
int CEpollThread::doSendMessage(SOCKET_KEY* pkey)
{
    ...
    while (itersend->second->size() > 0)
    {
        ...
        if (!pdata)
        {
            LOG(_WARN_, "CEpollThread::doSendMessage(), send message data is empty, fd=%d", pkey->fdkey);
            itersend->second->pop_front();
            continue;
        }

        if (!((itermap->second->key->fdkey == pdata->fddat) && (itermap->second->key->connect_time == pdata->connect_time)))
        {
        ...
        if (nsend < 0)
        {
            LOG(_ERROR_, "CEpollThread::doSendMessage() send_msg() error, fd=%d, time=%u, peerip=%s, port=%d, err=%s",
                pdata->fddat, pdata->connect_time, GETNULLSTR(itermap->second->peer_ip), itermap->second->peer_port, strerror(errno));
        ...
        else if (nsend == 0)
        {
            ...
            if (num > 0)
            {
                LOG(_INFO_, "CEpollThread::doSendMessage() send part message success, fd=%d, time=%u, peerip=%s, port=%d",
                    pdata->fddat, pdata->connect_time, GETNULLSTR(itermap->second->peer_ip), itermap->second->peer_port);
...
void CEpollThread::createClientCloseMsg(SOCKET_SET* psockset)
{
    ...
    {
        LOG(_ERROR_, "CEpollThread::createClientCloseMsg() error, msg.Out(buf, buflen) failed, fd=%d, time=%u, type=%d", psockset->key->fdkey, psockset->key->connect_time, psockset->type);
        msg.Debug();
        return;
    }
    ...
    if (!pdata)
    {
        LOG(_ERROR_, "CEpollThread::createClientCloseMsg() error, create NET_DATA failed, fd=%d, time=%u, type=%d", psockset->key->fdkey, psockset->key->connect_time, psockset->type);
        ::exit(-1);
    }

    if (!pdata->init(psockset->key->fdkey, psockset->key->connect_time, psockset->peer_ip, psockset->peer_port, psockset->type, buflen))
    {
        LOG(_ERROR_, "CEpollThread::createClientCloseMsg() error, pdata->init(fd...) failed, fd=%d, time=%u, type=%d", psockset->key->fdkey, psockset->key->connect_time, psockset->type);
        delete pdata;
        return;
    }
...
bool CEpollThread::addSocketToMap(SOCKET_SET* psockset)
{
    ...
    {
        LOG(_ERROR_, "CEpollThread::addSocketToMap() error, the epoll poll is full, fd=%d, peerip=%s, port=%d", psockset->key->fdkey, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
        return false;
    }
    ...
    if (iter != m_socketmap.end())
    {
        ...
        {
            LOG(_WARN_, "CEpollThread::addSocketToMap() error, found timeout connect, fd=%d, peerip=%s, port=%d", psockset->key->fdkey, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
        }
        else
        {
            ...
            case CLOSE_CLIENT:
                {
                    if (pdata->data != NULL)
                    {
                        ...
                        LOG(_INFO_, "CEpollThread::doSystemEvent(), closeClient(), fd=%d, conn_time=%u", pkey->fdkey, pkey->connect_time);
                        closeClient(pkey->fdkey, pkey->connect_time);
                        delete pkey;
                    }
                    break;
            ...
            case ADD_CLIENT:
                {
                    ...
                    if (!addClientToEpoll(psockset))
                    {
                        LOG(_ERROR_, "CEpollThread::doSystemEvent() error, addClientToEpoll() failed, fd=%d, time=%u, peer_ip=%s, port=%d",
                            psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
                        close(psockset->key->fdkey);
                        delete psockset;
                        break;
                    }

...
bool CEpollThread::startup() {
    ...
    if (!start()) {
        LOG(_ERROR_,
            "CEpollThread::startup() error, start() failed, m_epollfd=%d",
            m_epollfd);
        return false;
    }
...
void CEpollThread::doEpollEvent() {
    ...
    while (m_bOperate) {
        ...
        for (int i = 0; i < nevent; ++i) {
            ...
            if (m_events[i].events & EPOLLIN) {
                ...
                if (doSendMessage(pkey) < 0) {
                    ...
                    wev.data.ptr = pkey;
                    if (epoll_ctl(m_epollfd, EPOLL_CTL_MOD, pkey->fdkey, &wev) <
                        0) {
                        LOG(_ERROR_,
                            "CEpollThread::doEpollEvent() error, "
        ...
        for (std::map<int, std::list<NET_DATA *> *>::iterator itersendmap =
            ...
            if (epoll_ctl(m_epollfd, EPOLL_CTL_MOD, itersendmap->first, &ev) <
                0) {
                LOG(_ERROR_,
...
bool CEpollThread::doListen() {
    ...
    do {
        ...
        int tmp = 1;
        if (setsockopt(m_listenfd, SOL_SOCKET, SO_REUSEADDR, &tmp,
                       sizeof(tmp)) < 0) {
            LOG(_ERROR_,
                "CEpollThread::doListen() setsockopt(m_listenfd...) error");
            break;
        }

        if (!setNonBlock(m_listenfd)) {
            LOG(_ERROR_,
                "CEpollThread::doLIsten() setNonBlock(m_listenfd) error");
            break;
        }
        ...
        if (listen(m_listenfd, 4096) < 0) {  // max pending queue
            LOG(_ERROR_,
                "CEpollThread::doListen() listen(m_listenfd...) error");
            break;
        }
        ...
        if (epoll_ctl(m_epollfd, EPOLL_CTL_ADD, m_listenfd, &ev) < 0) {
            LOG(_ERROR_,
                "CEpollThread::doListen() epoll_ctl(m_epollfd...) error");
            break;
        }
...
bool CEpollThread::doAccept(int fd) {
    ...
    while (true) {
        ...
        int opt = 1;
        if (setsockopt(connfd, IPPROTO_TCP, TCP_NODELAY, &opt, sizeof(opt)) <
            0) {
            LOG(_ERROR_,
                "CEpollThread::doAccept(fd) setsockopt(connfd...) error, "
                "connfd=%d, peerip=%s, port=%d, error=%s",
...
int CEpollThread::doSendMessage(SOCKET_KEY *pkey) {
    ...
    if (itermap == m_socketmap.end() || itermap->second == NULL ||
        ...
        LOG(_ERROR_,
            "CEpollThread::doSendMessage() error, not found socket, fd=%d",
            pkey->fdkey);
    ...
    while (itersend->second->size() > 0) {
        ...
        if (nsend < 0) {
            LOG(_ERROR_,
                ...
                GETNULLSTR(itermap->second->peer_ip),
...
bool CEpollThread::parsePacketToRecvQueue(SOCKET_SET *psockset, char *buf,
    ...
    if (!psockset || !psockset->key) {
        LOG(_ERROR_,
            "CEpollThread::parsePacketToRecvQueue() error, param is null");
        return false;
    }

    if (buf == NULL || buflen <= 0) {
        LOG(_ERROR_,
            "CEpollThread::parsePackateToRecvQueue() error, buflen < 0");
        return false;
    }
...
void CEpollThread::closeClient(int fd, time_t conntime) {
    ...
    if (iter == m_socketmap.end()) {
        ...
        if (iter->second != NULL) {
            ...
            LOG(_ERROR_,
                "CEpollThread::closeClient(), the found socket is NULL");
        }
    ...
    while (pevent->outQueueWithoutLock(pdata, true)) {
        ...
        switch (pdata->eventid) {
            case CLOSE_CLIENT: {
                if (pdata->data != NULL) {
                    SOCKET_KEY *pkey =
                        reinterpret_cast<SOCKET_KEY *>(pdata->data);
                    LOG(_INFO_,
            ...
            case ADD_CLIENT: {
                ...
                SOCKET_SET *psockset =
                    reinterpret_cast<SOCKET_SET *>(pdata->data);
            ...
            default: {
                LOG(_ERROR_,
                    "CEpollThread::doSystemEvent() error, invalid event");
                if (pdata->data != NULL)

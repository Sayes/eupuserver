...
void CWorkBaseThread::run() {
  ...
  if (precvlist == NULL || peventlist == NULL) {
    LOG(_ERROR_,
        "CWorkBaseThread::run() error, recvlist == NULL || eventlist == NULL");
    m_bOperate = false;
    m_bIsExit = true;
    return;
  }
  ...
  while (m_bOperate) {
    ...
    while (precvlist->outQueue(pdata, true)) {
      if (pdata == NULL) {
        LOG(_ERROR_, "CWorkBaseThread::run() error, precvlist->outQueue(pdata) "
                     "get NULL data");
        continue;
      }
      ...
      if (nret < 0) {
        ...
        if (!CGlobalMgr::getInstance()->createCloseConnectEvent(
          ...
          LOG(_ERROR_, "CWorkBaseThread::run() error, "

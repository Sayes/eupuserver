...
bool CGlobalMgr::createMsgToSendList(int fd, time_t conntime,
  ...
  if (pstream) {
    if (!pstream->Out(reinterpret_cast<BYTE *>(pdata->pdata + NET_HEAD_SIZE),
                      buflen)) {
      LOG(_ERROR_, "CGlobalMgr::createMsgToSendList() error, param "
                   "stream->Out() failed");
      delete pdata;
      return false;
    }
  ...
  if (pdata->data_len > MAX_SEND_SIZE) {
    LOG(_ERROR_, "CGlobalMgr::createMsgToSendList() error, data size > "
  ...
  if (!msgHead.Out(reinterpret_cast<BYTE *>(pdata->pdata), buflen)) {
    LOG(_ERROR_, "CGlobalMgr::createMsgToSendList() error, "
...
bool CGlobalMgr::addMsgToSendList(NET_DATA *pmsg) {
  if (pmsg == NULL || m_pcursendmap == NULL) {
    LOG(_ERROR_, "CGlobalMgr::addMsgToSendList() error, param NET_DATA == NULL "
                 "|| m_pcursendmap == NULL");
    return false;
  }
  ...
  if (iter == m_pcursendmap->end()) {
    ...
    if (!plst) {
      LOG(_ERROR_, "CGlobalMgr::addMsgToSendList() error, net "
    ...
    if (plst) {
      LOG(_ERROR_, "CGlobalMgr::addMsgToSendList() error, found "
...
void CGlobalMgr::setServerSocket(int fd, time_t conntime, const std::string &ip,
  ...
  default: {
    LOG(_DEBUG_, "CGlobalMgr::setServerSocket(), invalid server type, fd=%d, "
...
void CGlobalMgr::switchSendMap() {
  if (m_pcursendmap == NULL || m_pbaksendmap == NULL) {
    LOG(_ERROR_, "CGlobalMgr::switchSendMap() error, m_pcursendmap == NULL || "
                 "m_pbaksendmap == NULL");
    return;
  }
  ...
  if (m_pcursendmap->size() > 0 || m_pbaksendmap->size() > 0)
    LOG(_INFO_, "CGlobalMgr::switchSendMap(), m_pcursendmap->size()=%d "
  ...
  for (; iterbak != m_pbaksendmap->end(); ++iterbak) {
    ...
    if (itercur != m_pcursendmap->end()) {
      ...
      if (itercur->second->size() + iterbak->second->size() >
        ...
        iterbak->second = NULL;
        LOG(_ERROR_, "CGlobalMgr::switchSendMap(), send list full, delete %d "
      ...
      m_pcursendmap->insert(std::map<int, std::list<NET_DATA *> *>::value_type(
...
bool CGlobalMgr::sendMsgToServer(int ntype, uint16_t mainid,
  ...
  if (pdata->fddat >= 0) {
    bret = createMsgToSendList(pdata->fddat, pdata->connect_time,
                               pdata->peer_ip, pdata->peer_port, ntype, mainid,
                               assistantid, code, reserve, stream, nlen);
  }
...
void CGlobalMgr::createServerConnect(int ntype) {
  ...
  default: {
    LOG(_ERROR_,
        "CGlobalMgr::createServerConnect() error, invalid server type, type=%d",
        ntype);
    break;
  }
  } // end switch
  ...
  if (pserver == NULL) {
    LOG(_ERROR_, "CGlobalMgr::createServerConnect() error, not found config "
  ...
  if (!m_eventlist.inQueue(pevent, false)) {
    LOG(_ERROR_, "CGlobalMgr::createServerConnect() error, "
  ...
  LOG(_INFO_, "CGlobalMgr::createServerConnect() end, fd=%d, time=%u, ip=%s, "
...
bool CGlobalMgr::createCloseConnectEvent(int fd, time_t conntime) {
  ...
  if (pkey == NULL || pevent == NULL) {
    LOG(_ERROR_, "CGlobalMgr::createCloseConnectEvent() error, _new SOCKET_KEY "
  ...
  if (!m_eventlist.inQueue(pevent, false)) {
    LOG(_ERROR_, "CGlobalMgr::createCloseConnectEvent() error, "

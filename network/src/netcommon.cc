...
SOCKET_SET *initSocketset(int fd, time_t conntime, const std::string &peerip, uint16_t peerport,
                          int ntype) {
  ...
  LOG(_INFO_, "initSocketset() end, fd=%d, time=%u, ip=%s, port=%d, type=%d", fd, conntime,
      peerip.c_str(), peerport, ntype);
  return socketset;
}
...
bool setNonBlock(int sockfd) {
  ...
  if (ioctlsocket(sockfd, FIONBIO, &mode) == SOCKET_ERROR) {
    LOG(_ERROR_, "setNonBlock() error, ioctlsocket() failed, sockfd=%d, error=%ld", sockfd,
        WSAGetLastError());
    return false;
  }
#endif
...
int doNonblockConnect(PCONNECT_SERVER pserver, int timeout, const std::string &localip) {
  ...
  if (setsockopt(fd, SOL_SOCKET, SO_SNDBUF, reinterpret_cast<const char *>(&nsend), sizeof(nsend)) <
      0) {
  ...
  if (setsockopt(fd, SOL_SOCKET, SO_RCVBUF, reinterpret_cast<const char *>(&nrecv), sizeof(nrecv)) <
      0) {
  ...
  int opt = 1;
  if (setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char *>(&opt), sizeof(opt)) <
      0) {
  ...
  int nret = connect(fd, reinterpret_cast<struct sockaddr *>(&addr), sizeof(addr));
  if (nret == 0) {
  ...
  nret = getsockopt(fd, SOL_SOCKET, SO_ERROR, reinterpret_cast<char *>(&sock_err),

...
bool CGlobalMgr::createMsgToSendList(int fd, time_t conntime, const std::string &ip, uint16_t port,
                                     int ntype, uint16_t mainid, uint16_t assistantid, BYTE code,
                                     BYTE reserve, CEupuStream *pstream, uint32_t nlen) {
  if (fd < 0) {
  ...
  if (pstream) {
    if (!pstream->Out(reinterpret_cast<BYTE *>(pdata->pdata + NET_HEAD_SIZE), buflen)) {
      LOG(_ERROR_,
...
void CGlobalMgr::clean() {
  ...
  for (; iter != m_pcursendmap->end(); ++iter) {
    std::list<NET_DATA *> *plst = iter->second;
    if (plst) {
      for (std::list<NET_DATA *>::iterator iterdata = plst->begin(); iterdata != plst->end();
           ++iterdata) {
        if (*iterdata) {
          delete *iterdata;
          *iterdata = NULL;
...
void CGlobalMgr::setServerSocket(int fd, time_t conntime, const std::string &ip, uint16_t port,
                                 int ntype) {
  m_serverlock.Lock();
  ...
  for (; iterbak != m_pbaksendmap->end(); ++iterbak) {
    ...
    if (itercur != m_pcursendmap->end()) {
      ...
      if (itercur->second->size() + iterbak->second->size() > (uint32_t)m_nMaxSendList) {
        ...
        for (std::list<NET_DATA *>::iterator itersend = iterbak->second->begin();
        ...
        itercur->second->insert(itercur->second->begin(), iterbak->second->begin(),
                                iterbak->second->end());
        iterbak->second->clear();
        delete iterbak->second;
        iterbak->second = NULL;
      ...
      m_pcursendmap->insert(
          std::map<int, std::list<NET_DATA *> *>::value_type(iterbak->first, iterbak->second));
    }
  }  // end for
...
void CGlobalMgr::setUserCenterSocket(int fd, time_t conntime, const std::string &peerip,
...
void CGlobalMgr::setMainSocket(int fd, time_t conntime, const std::string &peerip,
                               uint16_t peerport, int ntype) {
  m_mainkey.fddat = fd;
...
void CGlobalMgr::setLogSocket(int fd, time_t conntime, const std::string &peerip, uint16_t peerport,
                              int ntype) {
  m_logkey.fddat = fd;
...
void CGlobalMgr::setDistributeSocket(int fd, time_t conntime, const std::string &peerip,
                                     uint16_t peerport, int ntype) {
...
bool CGlobalMgr::sendMsgToServer(int ntype, uint16_t mainid, uint16_t assistantid, BYTE code,
                                 BYTE reserve, CEupuStream *stream, uint32_t nlen, bool blocked) {
  ...
  if (pdata->fddat >= 0) {
    bret = createMsgToSendList(pdata->fddat, pdata->connect_time, pdata->peer_ip, pdata->peer_port,
...
void CGlobalMgr::createServerConnect(int ntype) {
  ...
  int fd = doNonblockConnect(pserver, 3, CGlobalConfig::get_instance()->getListenIp());
  ...
  LOG(_INFO_,
      ...
      pdata->fddat, pdata->connect_time, pdata->peer_ip.c_str(), pdata->peer_port, pdata->type,
      pdata->data_len);
}

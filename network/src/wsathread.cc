...
bool CWSAThread::startup() {
  ...
  if (!m_recvbuffer) {
    LOG(_ERROR_,
        "CWSAThread::startup() error, _new char[m_recvbuflen] failed, "
        "m_recvbuflen=%d",
        m_recvbuflen);
    ::exit(-1);
  }
...
void CWSAThread::doWSAEvent() {
  ...
  while (m_bOperate) {
    for (std::unordered_map<int, SOCKET_SET *>::iterator itersockmap = m_socketmap.begin();
    ...
    error, (nIndex == WSA_WAIT_FAILED)");
                            ...
                            else
                            {
                                    ...
                                    if
    (::WSAEnumNetworkEvents(m_sockArray[i],
    m_eventArray[i], &event) == SOCKET_ERROR)
                                    {
                                            LOG(_ERROR_,
    "CWSAThread::doWSAEvent() error, ::WSAEnumNetworkEvents() failed,
    error=%s",
    WSAGetLastError());
                                            continue;
                                    }
    ...
    WSA_MAXIMUM_WAIT_EVENTS)
                                                    {
                                                            LOG(_INFO_,
    "CWSAThread::doWSAEvent() error, m_nEventTotal >
    WSA_MAXIMUM_WAIT_EVENTS");
                                                            continue;
                                                    }
    ...
    if (m_recvtmplst.size() > 0) {
      ...
      for (; iter != m_recvtmplst.end(); ++iter) {
        ...
        if (!precvlist->inQueueWithoutLock(*iter, false)) {
          LOG(_ERROR_,
              "CWSAThread::doWSAEvent() error, inQueueWithoutLock() "
              "failed");
          delete (*iter);
        }
      }
    ...
    for (std::map<int, std::list<NET_DATA *> *>::iterator itersendmap = psendmap->begin();
      ...
      if (itersockmap == m_socketmap.end() || itersockmap->second == NULL ||
        ...
        LOG(_ERROR_,
            "CWSAThread::doWSAEvent() error, m_socketmap.find(fd=%d) "
            "failed, itersockmap == m_socketmap.end() %s, "
            "itersockmap->second == NULL %s, itersockmap->second->key "
            "== NULL %s",
            itersendmap->first, itersockmap == m_socketmap.end() ? "true" : "false",
      ...
      // SOCKET_ERROR)
      //{
      //    LOG(_ERROR_, "CWSAThread::doEpollEvent() error,
      //    WSAEventSelect()
      //    failed, listen fd=%d, error=%ld", m_listenfd,
      //    WSAGetLastError());
      //    break;
      //}
    }
    ...
    if (itersockmap != m_socketmap.end()) {
      if (itersockmap->second != NULL) {
        if (itersockmap->second->key != NULL) {
          bclosed = true;
          // LOG(_ERROR_, "CWSAThread::doKeepaliveTimeout() error,
          // close
          // socket_set for timeout, fd=%d, time=%u, peerip=%s,
          // port=%d",
          //    itersocket->first,
          //    itersocket->second->key->connect_time,
      ...
      if (!bclosed) {
        LOG(_ERROR_,
            "CWSAThread::doKeepaliveTimeout() error, close "
...
bool CWSAThread::doListen() {
  ...
  do {
    ...
    int tmp = 1;
    if (setsockopt(m_listenfd, SOL_SOCKET, SO_REUSEADDR, (char *)&tmp, sizeof(tmp)) ==
        SOCKET_ERROR) {
      LOG(_ERROR_,
          "CWSAThread::doListen() error, setsockopt() failed, listen "
          "fd=%d, error=%ld",
    ...
    if (!setNonBlock(m_listenfd)) {
      LOG(_ERROR_,
          "CWSAThread::doListen() error, setNonBlock() failed, listen "
          "fd=%d",
          m_listenfd);
      break;
    }
    ...
    if (::bind(m_listenfd, (sockaddr *)&serv_addr, sizeof(serv_addr)) == SOCKET_ERROR) {
      LOG(_ERROR_,
          "CWSAThread::doListen() error, bind() failed, listen fd=%d, "
          "error=%ld",
          m_listenfd, WSAGetLastError());
      break;
    }
    ...
    if (::listen(m_listenfd, 4096) == SOCKET_ERROR) {
      LOG(_ERROR_,
          "CWSAThread::doListen() error, listen() failed, listen "
          "fd=%d, error=%ld",
          m_listenfd, WSAGetLastError());
      break;
    }
    ...
    if (!m_listenkey) {
      LOG(_ERROR_,
          "CWSAThread::doListen() error, _new SOCKET_KEY failed, listen "
          "fd=%d",
          m_listenfd);
      ::closesocket(m_listenfd);
      m_listenfd = INVALID_SOCKET;
    ...
    //{
    //    LOG(_ERROR_, "CWSAThread::doListen() error, _new SOCKET_KEY
    //    failed,
    ...
    // if (::WSAEventSelect(m_listenfd, event, FD_ACCEPT | FD_READ |
    // FD_CONNECT
    // | FD_CLOSE) == SOCKET_ERROR)
    //{
    //    LOG(_ERROR_, "CWSAThread::doListen() error, WSAEventSelect()
    //    failed,
...
bool CWSAThread::doAccept(int fd) {
  ...
  while (true) {
    ...
    if (setsockopt(connfd, SOL_SOCKET, SO_SNDBUF, (char *)&m_sendbufsize, sizeof(m_sendbufsize)) ==
        SOCKET_ERROR) {
      LOG(_ERROR_,
          "CWSAThread::doAccept() error, setsockopt(SO_SNDBUF=%d) "
    ...
    if (setsockopt(connfd, SOL_SOCKET, SO_RCVBUF, (char *)&m_readbufsize, sizeof(m_readbufsize)) ==
        SOCKET_ERROR) {
      LOG(_ERROR_,
    ...
    if (setsockopt(connfd, IPPROTO_TCP, TCP_NODELAY, (char *)&opt, sizeof(opt)) < 0) {
      LOG(_ERROR_,
          "CWSAThread::doAccept() error, setsockopt(TCP_NODELAY) "
          "failed, fd=%d, connfd=%d, error=%d",
    ...
    if (!psockset) {
      LOG(_ERROR_,
          "CWSAThread::doAccept() error, initSocketset() failed, "
    ...
    if (!addClientToWSA(psockset)) {
      LOG(_ERROR_,
          "CWSAThread::doAccept() error, addClientToWSA() failed, "
          "fd=%d, connfd=%d, peerip=%s, port=%d",
          fd, connfd, GETNULLSTR(peerip), port);
      delete psockset;
      ::closesocket(connfd);
...
bool CWSAThread::addClientToWSA(SOCKET_SET *psockset) {
  ...
  if (!addSocketToMap(psockset)) {
    LOG(_ERROR_,
        "CWSAThread::addClientToWSA() error, addSocketToMap() failed, "
...
void CWSAThread::doRecvMessage(SOCKET_KEY *pkey) {
  ...
  if (itersockmap == m_socketmap.end() || itersockmap->second == NULL ||
      itersockmap->second->key == NULL) {
    LOG(_ERROR_,
        "CWSAThread::doRecvMessage() error, can't find socket in map, "
        "fd=%d",
        pkey->fdkey);
    closeClient(pkey->fdkey, pkey->connect_time);
  ...
  if (itersockmap->second->key != pkey) {
    LOG(_ERROR_,
        "CWSAThread::doRecvMessage() error, the found socket dones't "
        "match pkey, fd=%d",
        pkey->fdkey);
    closeClient(pkey->fdkey, pkey->connect_time);
    // TODO check here, will return or not
  ...
  while (1) {
    ...
    if (nret < 0) {
      LOG(_ERROR_,
          "CWSAThread::doRecvMessage() error, recv_msg() failed, "
    ...
    if (buflen > 0) {
      ...
      if (!bparse) {
        LOG(_ERROR_,
            "CWSAThread::doRecvMessage() error, "
    ...
    if (nret == 0) {
      LOG(_ERROR_,
          "CWSAThread::doRecvMessage() error, recv_msg() return 0, "
          "fd=%d, time=%u, peerip=%s, port=%d",
          psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip),
          psockset->peer_port);
    }
...
int CWSAThread::doSendMessage(SOCKET_KEY *pkey) {
  ...
  if (itersockmap == m_socketmap.end() || itersockmap->second == NULL ||
      itersockmap->second->key == NULL) {
    LOG(_ERROR_,
        "CWSAThread::doSendMessage() error, do not find socket in "
        "m_socketmap, fd=%d",
        pkey->fdkey);
  ...
  if (itersendmap == psendmap->end()) {
    LOG(_WARN_,
        "CWSAThread::doSendMessage(), do not find data in m_sendmap, "
        "fd=%d, mapsize=%d",
  ...
  if (itersendmap->second == NULL) {
    LOG(_DEBUG_,
        "CWSAThread::doSendMessage() error, data in m_sendmap is NULL, "
        "fd=%d",
        pkey->fdkey);
  ...
  while (itersendmap->second->size() > 0) {
    ...
    if (nsend < 0) {
      ...
      if (num > 0) {
        LOG(_INFO_,
            "CWSAThread::doSendMessage() error, send_msg() send part "
            "of data, fd=%d, len=%d",
            pdata->fddat, buflen);
        ...
        LOG(_INFO_,
            "CWSAThread::doSendMessage() successed, fd=%d, time=%u, "
            "peerip=%s, port=%d",
            pdata->fddat, pdata->connect_time, GETNULLSTR(pdata->peer_ip), pdata->peer_port);
...
bool CWSAThread::parsePacketToRecvQueue(SOCKET_SET *psockset, char *buf, int buflen) {
  if (psockset == NULL || psockset->key == NULL) {
    LOG(_ERROR_,
        "CWSAThread::parsePacketToRecvQueue() error, psockset == NULL "
        "|| psockset->key == NULL");
    return false;
  }
  ...
  if (buf == NULL || buflen <= 0) {
    LOG(_ERROR_,
        "CWSAThread::parsePacketToRecvQueue() error, buf == NULL || "
        "buflen <= 0");
  ...
  while (curpos < buflen) {
    ...
    if (nlen > MAX_SEND_SIZE || nlen < NET_HEAD_SIZE) {
      LOG(_ERROR_,
          "CWSAThread::parsePacketToRecvQueue() error, invalid "
    ...
    if (pdata == NULL) {
      LOG(_ERROR_,
          "CWSAThread::parsePacketToRecvQueue() error, _new NET_DATA "
    ...
    if (!pdata->init(psockset->key->fdkey, psockset->key->connect_time, psockset->peer_ip,
      ...
      LOG(_ERROR_,
          "CWSAThread::parsePacketToRecvQueue() error, pdata->init() "
          "failed, fd=%d, time%u, peerip=%s, port=%d",
          psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip),
          psockset->peer_port);
      LOGHEX(_DEBUG_, "recv message:", buf, buflen);
...
void CWSAThread::closeClient(int fd, time_t conn_time) {
  ...
  //{
  //    LOG(_ERROR_, "CWSAThread::closeClient() error, WSAEventSelect(0)
  //    failed,
  //    fd=%d", fd);
  //}
  // if (m_keymap[i])
  ...
  if (itersockmap == m_socketmap.end()) {
    LOG(_ERROR_,
        "CWSAThread::closeClient() error, can not find socket in "
        "m_socketmap, fd=%d",
        fd);
  } else {
    if (itersockmap->second != NULL) {
      ...
      LOG(_ERROR_,
          "CWSAThread::closeClient() error, the found socket is NULL, "
          "fd=%d",
          fd);
    }
    m_socketmap.erase(itersockmap);
...
void CWSAThread::createClientCloseMsg(SOCKET_SET *psockset) {
  if (psockset == NULL || psockset->key == NULL) {
    LOG(_ERROR_,
        "CWSAThread::createClientCloseMsg() error, psockset == NULL "
        "|| psockset->key == NULL");
    return;
  }
  ...
  if (!msg.Out((BYTE *)buf, buflen)) {
    LOG(_ERROR_,
        "CWSAThread::createClientCloseMsg() error, msg.Out() failed, "
        "fd=%d, time=%u, peerip=%s, port=%d",
        psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip),
        psockset->peer_port);
    msg.Debug();
  ...
  if (pdata == NULL) {
    LOG(_ERROR_,
        "CWSAThread::createClientCloseMsg() error, _new NET_DATA "
        "failed, fd=%d, time=%u, peerip=%s, port=%d",
        psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip),
        psockset->peer_port);
    ::exit(-1);
  ...
  if (!pdata->init(psockset->key->fdkey, psockset->key->connect_time, psockset->peer_ip,
    ...
    LOG(_ERROR_,
        "CWSAThread::createClientCloseMsg() error, pdata->init() "
        "failed, fd=%d, time=%u, peerip=%s, port=%d",
        psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip),
        psockset->peer_port);
    delete pdata;
...
bool CWSAThread::addSocketToMap(SOCKET_SET *psockset) {
  if (psockset == NULL || psockset->key == NULL || psockset->key->fdkey < 0) {
    LOG(_ERROR_,
        "CWSAThread::addSocketToMap() error, psockset == NULL || "
  ...
  //{
  //    LOG(_ERROR_, "CWSAThread::addSocketToMap() error, the wsa event
  //    handler
  ...
  if (itersockmap != m_socketmap.end()) {
    if (itersockmap->second && itersockmap->second->key) {
      LOG(_WARN_,
          "CWSAThread::addSocketToMap() error, found timeout socket in "
          "m_socketmap, fd=%d",
          psockset->key->fdkey);
    } else {
      LOG(_WARN_,
  ...
  LOG(_INFO_,
      "CWSAThread::addSocketToMap() successed, fd=%d, time=%u, ip=%s, "
      "port=%d, type=%d",
  ...
  while (pevent->outQueueWithoutLock(pdata, true)) {
    ...
    switch (pdata->eventid) {
      ...
      case ADD_CLIENT: {
        if (pdata->data == NULL) {
          LOG(_ERROR_,
              "CWSAThread::doSystemEvent() error, event data is "
              "NULL");
          break;
        }
        ...
        if (psockset->key == NULL) {
          LOG(_ERROR_,
              "CWSAThread::doSystemEvent() error, event data->key is "
              "NULL");
          delete psockset;
          break;
        }
        ...
        CGlobalMgr::getInstance()->setServerSocket(psockset->key->fdkey,
                                                   psockset->key->connect_time, psockset->peer_ip,
                                                   psockset->peer_port, psockset->type);
        LOG(_INFO_,
            "CWSAThread::doSystemEvent(), handle ADD_CLIENT event, "
            "fd=%d, time=%u, peerip=%s, port=%d",
            psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip),
            psockset->peer_port);
        ...
        if (!addClientToWSA(psockset)) {
          LOG(_ERROR_,
              "CWSATread::doSystemEvent() error, addClientToWSA() "
              "failed, fd=%d, time=%u",
      ...
      default: {
        LOG(_ERROR_,
            "CWSAThread::doSystemEvent() error, invalid event, "
            "event=%d",
            pdata->eventid);
        if (pdata->data != NULL) {
          delete pdata->data;
...
bool CWSAThread::createConnectServerMsg(SOCKET_SET *psockset) {
  ...
  if (!msg.Out((BYTE *)buf, buflen)) {
    LOG(_ERROR_,
        "CWSAThread::createConnectServerMsg() error, msg.Out() "
        "failed, fd=%d, time=%u, peerip=%s, port=%d",
        psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip),
        psockset->peer_port);
    msg.Debug();
  ...
  if (pdata == NULL) {
    LOG(_ERROR_,
        "CWSAThread::createConnectServerMsg() error, _new NET_DATA "
        "failed, fd=%d, time=%u, peerip=%s, port=%d",
        psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip),
        psockset->peer_port);
    ::exit(-1);

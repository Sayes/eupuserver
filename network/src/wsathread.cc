...
void CWSAThread::reset() {
    auto add2 =
        std::bind(add1, std::placeholders::_1, std::placeholders::_2, 10);
}
...
void CWSAThread::doWSAEvent() {
    ...
    while (m_bOperate) {
        for (std::unordered_map<int, SOCKET_SET *>::iterator itersockmap =
            ...
            if (m_listenfd == itersockmap->first) {
                if (!doAccept(m_listenfd)) {
                    LOG(_ERROR_,
                        "CWSAThread::doWSAEvent() error, m_listenfd=%d",
                        m_listenfd);
                }
                continue;
            } else {
        ...
        for (std::list<int>::iterator iterdelsendfdlist =
                 m_delsendfdlist.begin();
...
void CWSAThread::doKeepaliveTimeout() {
    ...
    for (; itersockmap != m_socketmap.end(); ++itersockmap) {
        if (itersockmap->second != NULL) {
            // TODO check here, > or < ?
            if (itersockmap->second->refresh_time + m_keepalivetimeout <
                curtime) {
...
bool CWSAThread::doListen() {
    ...
    do {
        if (m_listenfd == INVALID_SOCKET) {
            LOG(_ERROR_,
                "CWSAThread::doListen() error, create listen socket failed");
            break;
        }
    ...
    while (true) {
        ...
        if (connfd == INVALID_SOCKET) {
            if (WSAGetLastError() != WSAEWOULDBLOCK) {
                LOG(_ERROR_, "CWSAThread::doAccept() error, fd=%d, error=%d",
                    fd, WSAGetLastError());
            }
            return true;
        }
...
int CWSAThread::doSendMessage(SOCKET_KEY *pkey) {
    ...
    std::map<int, std::list<NET_DATA *> *> *psendmap =
        CGlobalMgr::getInstance()->getBakSendMap();
    std::map<int, std::list<NET_DATA *> *>::iterator itersendmap =
        ...
        if (nsend < 0) {
            ...
            if (num > 0) {
                ...
                LOG(_INFO_,
                    ...
                    GETNULLSTR(pdata->peer_ip), pdata->peer_port);
...
bool CWSAThread::addSocketToMap(SOCKET_SET *psockset) {
    ...
    LOG(_INFO_,
        ...
        "port=%d, type=%d",
        psockset->key->fdkey, psockset->key->connect_time,
        psockset->peer_ip.c_str(), psockset->peer_port, psockset->type);
...
void CWSAThread::deleteSendMsgFromSendMap(int fd) {
    ...
    if (itersendmap != psendmap->end()) {
        if (itersendmap->second != NULL) {
            std::list<NET_DATA *>::iterator iterdata =
                itersendmap->second->begin();
        ...
        switch (pdata->eventid) {
            case CLOSE_CLIENT: {
                if (pdata->data != NULL) {
                    ...
                    LOG(_INFO_,
                        "CWSATread::doSystemEvent() handle CLOSE_CLIENT");
                    closeClient(pkey->fdkey, pkey->connect_time);
                    delete pkey;
                }
                break;
...
bool CWSAThread::createConnectServerMsg(SOCKET_SET *psockset) {
    ...
    if (!pdata->init(psockset->key->fdkey, psockset->key->connect_time,
        ...
        LOG(_ERROR_,
            ...
            psockset->key->fdkey, psockset->key->connect_time,
            psockset->peer_ip, psockset->peer_port);
        return false;
    }

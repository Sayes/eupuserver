...
bool CWSAThread::addClientToWSA(SOCKET_SET* psockset)
{
    ...
    {
        LOG(_ERROR_, "CWSAThread::addClientToWSA() error, addSocketToMap() failed, fd=%d, peerip=%s, port=%d", psockset->key->fdkey, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
        return false;
    }
...
void CWSAThread::doRecvMessage(SOCKET_KEY* pkey)
{
    ...
    std::unordered_map<int, SOCKET_SET*>::iterator itersockmap = m_socketmap.find(pkey->fdkey);
    if (itersockmap == m_socketmap.end() || itersockmap->second == NULL || itersockmap->second->key == NULL)
    {
        LOG(_ERROR_, "CWSAThread::doRecvMessage() error, can't find socket in map, fd=%d", pkey->fdkey);
        closeClient(pkey->fdkey, pkey->connect_time);
    ...
    {
        LOG(_ERROR_, "CWSAThread::doRecvMessage() error, the found socket dones't match pkey, fd=%d", pkey->fdkey);
        closeClient(pkey->fdkey, pkey->connect_time);
        //TODO check here, will return or not
        //return;
    }
    ...
    while (1)
    {
        ...
        if (nret < 0)
        {
            LOG(_ERROR_, "CWSAThread::doRecvMessage() error, recv_msg() failed, fd=%d, time=%u, peerip=%s, port=%d", psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
            return;
        }
        ...
        if (buflen > 0)
        {
            ...
            if (!bparse)
            {
                LOG(_ERROR_, "CWSAThread::doRecvMessage() error, parsePacketToRecvQueue() failed, fd=%d, time=%u, peerip=%s, port=%d", psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
            }
        }

        if (nret == 0)
        {
            LOG(_ERROR_, "CWSAThread::doRecvMessage() error, recv_msg() return 0, fd=%d, time=%u, peerip=%s, port=%d", psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
        }
...
int CWSAThread::doSendMessage(SOCKET_KEY* pkey)
{
    ...
    if (itersockmap == m_socketmap.end() || itersockmap->second == NULL || itersockmap->second->key == NULL)
    {
        LOG(_ERROR_, "CWSAThread::doSendMessage() error, do not find socket in m_socketmap, fd=%d", pkey->fdkey);
    ...
    map<int, list<NET_DATA*>*>* psendmap = CGlobalMgr::getInstance()->getBakSendMap();
    map<int, list<NET_DATA*>*>::iterator itersendmap = psendmap->find(pkey->fdkey);
    if (itersendmap == psendmap->end())
    {
        LOG(_WARN_, "CWSAThread::doSendMessage(), do not find data in m_sendmap, fd=%d, mapsize=%d", pkey->fdkey, psendmap->size());
        return 0;
    }

    if (itersendmap->second == NULL)
    {
        LOG(_DEBUG_, "CWSAThread::doSendMessage() error, data in m_sendmap is NULL, fd=%d", pkey->fdkey);
    ...
    while (itersendmap->second->size() > 0)
    {
        ...
        if (!((itersockmap->second->key->fdkey == pdata->fddat) && (itersockmap->second->key->connect_time == pdata->connect_time)))
        {
            LOG(_ERROR_, "CWSAThread::doSendMessage() error, data and socket don't match in fd and connect_time, data fd=%d", pdata->fddat);
        ...
        if (nsend < 0)
        {
            LOG(_ERROR_, "CWSAThread::doSendMessage() error, send_msg() failed, fd=%d", pdata->fddat);
        ...
        else if (nsend == 0)
        {
            ...
            if (num > 0)
            {
                LOG(_INFO_, "CWSAThread::doSendMessage() error, send_msg() send part of data, fd=%d, len=%d", pdata->fddat, buflen);
            ...
            {
                LOG(_INFO_, "CWSAThread::doSendMessage() successed, fd=%d, time=%u, peerip=%s, port=%d", pdata->fddat, pdata->connect_time, GETNULLSTR(pdata->peer_ip), pdata->peer_port);
...
bool CWSAThread::parsePacketToRecvQueue(SOCKET_SET* psockset, char* buf, int buflen)
{
    ...
    while (curpos < buflen)
    {
        ...
        {
            LOG(_ERROR_, "CWSAThread::parsePacketToRecvQueue() error, invalid message size, fd=%d, time=%u, peerip=%s, port=%d", psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
            return false;
        }
        ...
        if (pdata == NULL)
        {
            LOG(_ERROR_, "CWSAThread::parsePacketToRecvQueue() error, _new NET_DATA failed, fd=%d, time=%u, peerip=%s, port=%d", psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
            //TODO check here, buf or buf + curpos
            LOGHEX(_DEBUG_, "recv message:", buf + curpos, buflen);
            ::exit(-1);
        }

        if (!pdata->init(psockset->key->fdkey, psockset->key->connect_time, psockset->peer_ip, psockset->peer_port, psockset->type, nlen))
        {
            LOG(_ERROR_, "CWSAThread::parsePacketToRecvQueue() error, pdata->init() failed, fd=%d, time%u, peerip=%s, port=%d", psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
            LOGHEX(_DEBUG_, "recv message:", buf, buflen);
            delete pdata;
            return false;
        }
...
void CWSAThread::createClientCloseMsg(SOCKET_SET* psockset)
{
    ...
    {
        LOG(_ERROR_, "CWSAThread::createClientCloseMsg() error, msg.Out() failed, fd=%d, time=%u, peerip=%s, port=%d", psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
        msg.Debug();
        return;
    }
    ...
    if (pdata == NULL)
    {
        LOG(_ERROR_, "CWSAThread::createClientCloseMsg() error, _new NET_DATA failed, fd=%d, time=%u, peerip=%s, port=%d", psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
        ::exit(-1);
    }

    if (!pdata->init(psockset->key->fdkey, psockset->key->connect_time, psockset->peer_ip, psockset->peer_port, psockset->type, buflen))
    {
        LOG(_ERROR_, "CWSAThread::createClientCloseMsg() error, pdata->init() failed, fd=%d, time=%u, peerip=%s, port=%d", psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
        delete pdata;
        return;
    }
...
bool CWSAThread::addSocketToMap(SOCKET_SET* psockset)
{
    ...
    std::unordered_map<int, SOCKET_SET*>::iterator itersockmap = m_socketmap.find(psockset->key->fdkey);
    if (itersockmap != m_socketmap.end())
    {
        if (itersockmap->second && itersockmap->second->key)
        {
            LOG(_WARN_, "CWSAThread::addSocketToMap() error, found timeout socket in m_socketmap, fd=%d", psockset->key->fdkey);
        }
        else
        {
            LOG(_WARN_, "CWSAThread::addSocketToMap() error, found timeout socket in m_socketmap, but sockset == NULL || key == NULL, fd=%d", psockset->key->fdkey);
        }
    ...
    m_socketmap.insert(std::unordered_map<int, SOCKET_SET*>::value_type(psockset->key->fdkey, psockset));

    LOG(_INFO_, "CWSAThread::addSocketToMap() successed, fd=%d, time=%u, ip=%s, port=%d, type=%d", psockset->key->fdkey, psockset->key->connect_time, psockset->peer_ip.c_str(), psockset->peer_port, psockset->type);
...
void CWSAThread::doSystemEvent()
{
    ...
    while (pevent->outQueueWithoutLock(pdata, true))
    {
        ...
        case ADD_CLIENT:
        {
            ...
            CGlobalMgr::getInstance()->setServerSocket(psockset->key->fdkey, psockset->key->connect_time, psockset->peer_ip, psockset->peer_port, psockset->type);
            LOG(_INFO_, "CWSAThread::doSystemEvent(), handle ADD_CLIENT event, fd=%d, time=%u, peerip=%s, port=%d", psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
            ...
            if (!addClientToWSA(psockset))
            {
                LOG(_ERROR_, "CWSATread::doSystemEvent() error, addClientToWSA() failed, fd=%d, time=%u", psockset->key->fdkey, psockset->key->connect_time);
                ::closesocket(psockset->key->fdkey);
                delete psockset;
                break;
            }
...
bool CWSAThread::createConnectServerMsg(SOCKET_SET* psockset)
{
    ...
    if (!msg.Out((BYTE*)buf, buflen))
    {
        LOG(_ERROR_, "CWSAThread::createConnectServerMsg() error, msg.Out() failed, fd=%d, time=%u, peerip=%s, port=%d", psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
        msg.Debug();
        return false;
    }
    ...
    if (pdata == NULL)
    {
        LOG(_ERROR_, "CWSAThread::createConnectServerMsg() error, _new NET_DATA failed, fd=%d, time=%u, peerip=%s, port=%d", psockset->key->fdkey, psockset->key->connect_time, GETNULLSTR(psockset->peer_ip), psockset->peer_port);
        ::exit(-1);
    }
    ...
    if (!pdata->init(psockset->key->fdkey, psockset->key->connect_time, psockset->peer_ip, psockset->peer_port, psockset->type, buflen))
    {
        LOG(_ERROR_, "CEpollThread::createConnectServerMsg() error, NET_DATA init() failed, fd=%d, conn_time=%u, peer_ip=%s, port=%d",
            psockset->key->fdkey, psockset->key->connect_time, psockset->peer_ip, psockset->peer_port);
        return false;
    }

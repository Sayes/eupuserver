...
int CWorkThread::processMessage(NET_DATA* pdata) {
    if (!pdata) {
        LOG(_ERROR_,
            "CWorkThread::processMessage() error, the message is null");
        return 0;
    }
    ...
    if (!header.In((BYTE*)pdata->pdata, uTmp)) {
        LOG(_ERROR_,
            "CWorkThread::processMessage() error, parse message head failed");
        return -1;
    }
    ...
    switch (header.uMainID) {
        ...
        case RS_CONNECTED: {
            LOG(_INFO_,
                "CWorkThread::processMessage(), deal with RS_CONNECTED");
            break;
        }
